...
'''DUC2005: This script aims to extract gold summary from one of ".pans"
files and then, write each summary to a seperate file. then copy this
files to their corresponding folder in destination folder where rouge
evaluation will occur.'''


def main():
    # path to the original dataset where system summaries generated based on these dataset.
    dataset_original_path = input(r'Enter the path to the original dataset where ' +
    ...
    main_path = input(r'Enter the path to the destination folder where rouge '
                      + 'evaluation will occur:\n')
    for file in listdir(path):
        gold_summ_path = join(path, file)
        if isfile(gold_summ_path):
            docset = file.split(sep='.')[0]
            if docset.lower() in new_list:
                summarizer_id = file.split(sep='.')[-1]
                docset_id = folders_name[new_list.index(docset.lower())]
    ...
    print('{}:    Copied Gold Summary to reference Folder'.format(docset.lower()))
